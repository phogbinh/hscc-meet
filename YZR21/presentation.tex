...
\begin{frame}
  ...
  \begin{itemize}
    \item One base station, downlink transmission, orthogonal frequency-division multiple access (OFDMA), eMBB and URLLC users.
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Close-in (CI) free space reference distance model
      ...
      \end{equation}
    \item Path loss exponent and shadowing standard deviation for urban macro-cellular (UMa) line-of-sight (LoS) over frequency and 3D distance ranging from 2 to 73.5\textcolor{red}{GHz} and 58 to 930\textcolor{red}{m} are derived via minimum mean square error (MMSE) fit by \cite{SRRTGKRKPJ16} as
      \begin{align}
        \pathLossExponent &= 2\\
